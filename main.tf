...
resource "aws_route" "nat_gateway" {
  ...
  nat_gateway_id         = var.single_nat_gateway ? aws_nat_gateway.private[0].id : aws_nat_gateway.private[count.index].id
  depends_on             = [aws_route_table.private, aws_nat_gateway.private]
}
